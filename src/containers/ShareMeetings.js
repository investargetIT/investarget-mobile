...
class ShareMeetings extends React.Component {
  ...
  render() {
    return <div>
      ...
      <div style={{ padding: 6 }}>
        {this.state.data.map(m => <div key={m.id} style={{ marginTop: 10, border: '1px solid lightgray', borderRadius: 6 }}>
          <div style={{ padding: 10 }}>
            <div style={{ fontSize: 15, fontWeight: 'bold', color: '#000' }}>{`${m.username}（${m.org && m.org.orgname}）`}</div>
